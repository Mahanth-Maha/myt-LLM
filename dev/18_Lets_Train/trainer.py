...
class DecodeTransformerTrainer:
    ...
    def train(self, max_steps=10000, num_epochs = 1, force_epochs=False, val_iters=100, save_steps=500):
        ...
        print(f'➡️ No of Epochs specified: {num_epochs} -> ~{num_epochs * len(self.train_loader)} iterations (micro-steps)')
        if not force_epochs:
    ...
    def save_checkpoint(self, step_or_iter, async_write=True, final_copy = False):
        ...
        if step != 0:
            ...
            if self.tokenizer:
                ...
                with open(os.path.join(plots_dir,"samples.txt"), "w") as f:
                    ...
                    f.write("="*120 + "\n")
                    f.write("<|PARSING_START|>" + "\n")
                    for p, o in zip(self.samples_cfg['prompts'], outputs):
                        f.write(f"<|INFERNCE={it:2}|>\n")
                        f.write(f"<|startofprompt|>\n{p}\n<|endofpromt|>\n")
                        f.write(f"<|startofresponse|>\n{o}\n<|endofresponse|>\n")
                        f.write("="*120 + "\n")
                        it += 1 
                    f.write("<|PARSING_STOP|>" + "\n")
        ...
        if step % self.tc.milestone == 0  and step != 0:
