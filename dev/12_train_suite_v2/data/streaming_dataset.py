...
def get_logger() -> logging.Logger:
    ...
    return setup_logging(cfg, name=__name__)
...
class ValShardDataset(Dataset):
    def __init__(self, shard_tensor, context_length):
        self.tokens = shard_tensor
        self.context_length = context_length
        self.samples = len(self.tokens) - context_length

    def __len__(self):
        return max(0, self.samples)

    def __getitem__(self, idx):
        seq = self.tokens[idx: idx + self.context_length + 1]
        return {
            'input_ids': seq[:-1],
            'labels': seq[1:]
        }
...
def get_val_loader(shard_folder,
                   context_length,
                   batch_size,
                   num_workers=0,
                   pin_memory=False):

    folder = Path(shard_folder)
    shard_files = sorted(folder.rglob("shard_*.pt"),
                         key=lambda p: (p.parent.name, p.name))
    if not shard_files:
        raise FileNotFoundError(f"No shards found in {shard_folder}")

    last_shard_path = shard_files[-1]
    tokens = torch.load(last_shard_path, map_location='cpu')

    val_dataset = ValShardDataset(tokens, context_length)
    return DataLoader(
        val_dataset,
