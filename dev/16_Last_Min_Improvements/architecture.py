...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, model_dimension, context_length, n_heads, dropout=0.3):
        super().__init__()
        assert (model_dimension % n_heads == 0), "dim must be divisible by n_heads"
        assert (model_dimension // n_heads) % 2 == 0, "head_dim must be even for RoPE"
    ...
    def forward(self,x, start_pos=0):
        ...
        K = K.view(B, T, self.n_heads, self.each_head_size)
...
class DecoderBlock(nn.Module):
    def __init__(self, model_dimension, context_length, n_heads, ffn_hid_dim, non_linearity='gelu', dropout=0.3):
        super().__init__()
        self.attn_norm = RMSNorm(model_dimension)
        self.attn = MultiHeadSelfAttention(model_dimension, context_length, n_heads, dropout)
...
class DecoderOnlyTransformer(nn.Module):
    def __init__(self, vocab_size, context_length, model_dimension, n_heads, Nx_blocks, ffn_hid_dim, non_linearity='gelu', dropout=0.3, tie_weights=True, init_std_val = 0.02, use_checkpoint=False,checkpoint_ratio = 0.5):
        super().__init__()
