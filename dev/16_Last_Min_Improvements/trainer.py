...
class GeneralTrainer:
                ...
                for batch_idx, (x, y) in enumerate(self.train_loader):
                    ...
                    if self.precision_config.get('autocast_enabled', False):
                        ...
                        if self.precision_config.get('scaler'):
                            ...
                            self.precision_config['scaler'].unscale_(self.optimizer)
    ...
    def warmup_time_check_for_train(self, warmup_steps=100):
        ...
        avg_time_recent = times[-last_n:].mean()
        avg_time_all = times.mean()

        print(f"Average time per training step (all {len(times)} steps): {avg_time_all:.4f} seconds")
        print(f"Average time per training step (last {last_n} steps): {avg_time_recent:.4f} seconds [ <-- CONSIDER]")
