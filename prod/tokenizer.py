...
class mytlmTokenizer:
    ...
    def decode(self, tokens):
        ...
        cleaned_tkns = []
        for t in tokens:
            if not (t <= VALID_LAST_TOKEN or t in self.special_tokens.values()):
                print(f"⚠️ Warning: token {t} is out of valid range, replacing with {UNK_TOKEN}({UNK_TOKEN_IDX})")
                cleaned_tkns.append(UNK_TOKEN_IDX)
            else:
                cleaned_tkns.append(t)
        return self.encoder.decode(cleaned_tkns) if len(cleaned_tkns) > 0 else ""
